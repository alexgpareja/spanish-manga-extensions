...
var _Sources = (() => {
  ...
  // src/TmoManhwa/TmoManhwa.ts
  var TmoManhwa_exports = {};
  __export(TmoManhwa_exports, {
    TmoManhwa: () => TmoManhwa,
    TmoManhwaInfo: () => TmoManhwaInfo
  });
  ...
  var TmoManhwaInfo = {
    ...
    icon: "icon.png",
    author: "alexgpareja",
    description: "TmoManhwa \u2014 Manhwa y Manga en Espa\xF1ol",
  ...
  function getNumId(mangaId) {
    return mangaId.split("_")[1] ?? "";
  }
  function extractNumId(ogImage) {
  ...
  var TmoManhwa = class {
    ...
    async getMangaDetails(mangaId) {
      ...
      const resp = await this.requestManager.schedule(
        App.createRequest({ url: `${BASE_URL}/manhwa/${slug}/`, method: "GET" }),
        this.RETRIES
      );
      ...
      const desc = $('meta[name="description"]').attr("content") || $('meta[property="og:description"]').attr("content") || $('[class*="summary"] p, .entry-content p').first().text().trim() || "";
    ...
    async getChapters(mangaId) {
      ...
      $('ul.chapter-list li a[href*="capitulo"]').each((_, el) => {
        const href = $(el).attr("href") ?? "";
        const m = href.match(/capitulo-([0-9]+(?:\.[0-9]+)?)/);
        if (!m) return;
        const chapNum = parseFloat(m[1]);
        const chapId = `${numId}_${m[1]}`;
    ...
    async getViewMoreItems(_, metadata) {
      ...
      const $ = this.cheerio.load(resp.data);
      const hasNext = $('a.next, .pagination .next, [class*="paginat"] .next').length > 0 || $('a[href*="/page/"]').toArray().some((el) => {
    ...
    async getSearchResults(query, metadata) {
      ...
      if (term) {
        ...
        url = page === 1 ? `${BASE_URL}/genero/${genres[0]}/` : `${BASE_URL}/genero/${genres[0]}/page/${page}/`;
      } else {
        url = page === 1 ? `${BASE_URL}/biblioteca/` : `${BASE_URL}/biblioteca/page/${page}/`;
      }
      const resp = await this.requestManager.schedule(
    ...
    parseTiles($) {
      ...
      $('a[href*="/manhwa/"]').each((_, el) => {
        ...
        const image = img.attr("src") ?? img.attr("data-src") ?? `${BASE_URL}/uploads/${slug}-thumbnail.jpg`;
  ...
  return __toCommonJS(TmoManhwa_exports);
})();
